...
%%% Agregar el split de las particiones
% split(+Particiones, +NumEjs, -Split)
% Calcula el split de un conjunto de ejemplos particionados por un atributo.
split([], _, 0) :- !.
split([Part|Parts], NumEjs, Split) :-
    length(Part, NumEjsPart),
    split(Parts, NumEjs, RestoSplit),
    (NumEjsPart =:= 0 ->
        Split is RestoSplit;
        P is NumEjsPart / NumEjs,
        Split is RestoSplit - P * (log(P) / log(2))
    ).
...
eligeAtr(Ejs, Atrs, Atr, Vals, RestoAtrs) :-
    ...
    findall((Atr-Vals)/GainRatio,
            (member(Atr, Atrs),
             vals(Ejs, Atr, [], Vals),
             separaEnSubConjs(Vals, Ejs, Atr, Parts),
             informacionResidual(Parts, NumEjs, IR),
             Gain is I - IR,
             split(Parts, NumEjs, Split),
             (Split =:= 0 -> GainRatio = 0; GainRatio is Gain / Split)
            ),
            Todos),
