	  ...
	  (cond
	    ((equal file-ext "arff") 
	     (let ((attribs-doms (arff-get-attribs-doms file-lines)))
	       ...
	       (get-examples *data*)
	       (format t "Conjunto de entrenamiento inicializado desde ~s.~%" file)))
	    ((equal file-ext "csv") 
	     (let ((attribs-doms (csv-get-attribs-doms file-lines)))
	       ...
	       (get-examples *data*)
	       (format t "Conjunto de entrenamiento inicializado despues ~s.~%" file)))
...
;;; Reset
(defun reset ()
  "Resetear el entorno de aprendizaje"
  (setf *data* nil 
	*examples* nil
	*target* nil
	*attributes* nil
	*domains* nil
        *root* nil
	*gensym-counter* 1)
  (format t "La configuración del ID3 fue reseteada.~%"))
