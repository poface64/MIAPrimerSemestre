%%%% 1.- UNIFICACIÓN A MANO %%%%
% 1. Caso base: si los dos términos son iguales, no hay más sustituciones necesarias.
unificar(X, X, unifica) :- !.
% 2. Unificación de variables: si el primer término es una variable.
unificar(Var, Term, [(Var / Term)]) :-
    var(Var), % Verificar que el pprimer termino es una variable
    \+ Var == Term,  % La variable no debe ser identica al segundo termino
    !. % Corta para que no haga backtracking si esta regla se cumple
% 3. Unificación de variables: si el segundo término es una variable.
...
% 4. Unificación de términos compuestos: descomponer y unificar recursivamente.
unificar(f(T1, T2), f(S1, S2), _) :-
    unificar(T1, S1, _), % Unificación recursiva del primer componente.
    unificar(T2, S2, _). % Unificación recursiva del segundo componente.
% 5. Unificación falla si los términos no pueden unificarse.
